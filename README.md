...
### 深度依赖安全分析
本项目实现了一套完整的依赖安全分析系统，使用 yarn 在隔离环境中实际安装包，分析完整的依赖树中是否存在不安全的包：

- **隔离安装**: 在临时目录中使用 yarn 实际安装包，类似 WebContainer 的隔离方式
- **完整分析**: 分析所有已安装的包，包括所有传递性依赖，无深度限制
- **智能缓存**: 使用缓存机制避免重复安装和分析相同的包，提升性能
- **自动清理**: 分析完成后自动清理临时目录
- **自动更新**: 当插件版本更新时自动重新分析
...
3. 所有传递性依赖（通过实际安装的 node_modules 分析）
